...
const Post = ({
  ...
  const [liked, setLiked] = React.useState(
    !likedBy.every((like) => like.uid !== user.uid)
  );
  ...
  return (
    <div className=" bg-white rounded-lg  space-y-2 px-2 shadow-xl pt-2 mb-4">
      ...
      <div className="flex justify-around text-sm text-gray-700  font-normal border-t">
        {!liked ? (
          <div
            className="flex items-center gap-x-1 hover:cursor-pointer hover:text-black duration-75  px-1 md:px-2 py-1 md:py-3 my-1 hover:bg-blue-50 rounded-md  "
            onClick={updateLikes}
          >
          ...
          <div
            className="flex items-center gap-x-1 text-blue-500 hover:cursor-pointer duration-75  px-1 md:px-2 py-1 md:py-3 my-1  rounded-md  "
        ...
        <div
          className="flex items-center gap-x-1 hover:cursor-pointer hover:text-black duration-150   px-1 md:px-2 py-1 md:py-3 my-1 hover:bg-blue-50 rounded-md "
          onClick={() => {
            setShowComment(!showComment);
          }}
        >
          <BiCommentDots style={{ fontSize: "1.rem" }} />
          <span>Comments</span>
        </div>
        ...
        <div className="">
            ...
            <div className="flex flex-col gap-y-2 p-2 text-sm">
              {comnts.map((comnt, index) => {
                return (
                  <div
                    key={index}
                    className="flex border-t py-2 items-center gap-x-4"
                  >
          ...
          <div className="p-2 flex">
            ...
            <input
              type="text"
              className={`${
                loading ? `text-xs flex-1 text-gray-500 cursor-not-allowed` : ``
              }flex-1 ml-2 border rounded-full bg-white p-2`}
              disabled={loading}
      ...
      {showComment && comment.length > 0 ? (
        <div className="ml-14">
          <button
            className="text-white px-3 py-[2px]  mb-4 rounded-full text-sm hover:bg-blue-800 duration-150 bg-blue-500 font-semibold"
