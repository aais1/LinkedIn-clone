...
const RecentJobSearches = () => {
  ...
  return (
    <div className={`bg-white text-black rounded-md shadow-xl`}>
      <div
        className={` ${
          !showMore ? `max-h-[260px]` : `max-h-[600px]`
      ...
      >
        <div className="flex font-semibold justify-between p-1 md:px-4 md:py-2">
          ...
          <button className="text-gray-500 hover:bg-blue-50 p-1 rounded-md text-sm">
            Clear
          </button>
        </div>
      ...
      <div className="border-t flex justify-center">
        <button
          className="flex gap-x-2 items-center w-[130px] font-semibold text-blue-500  my-4 p-2 rounded-md hover:bg-blue-50"
          onClick={handleChange}
        >
          Show {`${!showMore ? `more` : `less`} `}{" "}
          <FaAngleDown
            className={` duration-700 ${!showMore ? "rotate-180" : ``}`}
          />
        </button>
      </div>
    </div>
